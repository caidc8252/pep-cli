...
> `--issuer` 目前必须带，而且**每次 `login` 都要带**——它只覆盖当次，不会被记住。
> 原因是生产地址上的授权服务还没部署完，得显式指到当前可用的环境。
>
> 「不记住」是刻意的：早期版本会把上次用过的环境参数存下来当下次的默认值，结果存量配置
> 悄悄压过新版本的内置值，登录一律失败且报错完全指不到本地配置。所以现在宁可每次多敲一个
> 参数。`login` 不是常跑的命令（令牌会自动续期），实际敲的次数很少。
>
> 等生产上线，这个参数整个不需要了——不带就是生产。
